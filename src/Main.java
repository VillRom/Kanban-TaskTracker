...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        taskmanager.addSubtask(new Subtask("Вытерить пыль", "на полках шкафа", StatusTasks.NEW, 1
                , 4));
        taskmanager.addSubtask(new Subtask("Помыть полы", "в комнате", StatusTasks.DONE, 2,
                4));
        ...
        Managers.getDefault().updateTask(3, new Task("Посмотреть урок анг. языка", "Урок №25"
                , StatusTasks.IN_PROGRESS, 3));
        Managers.getDefault().updateSubtask(1, new Subtask("Вытерить пыль", "на полках шкафа"
                , StatusTasks.DONE
                , 1, 4));
